...
def runMCS(rId,data):
    if len(RUNS_QUEUE)<=MAX_PARALLEL_RUNS:
        ...
        email=EmailMessage('MolOpt-Update','Your run finished you can download the result through the tasks section','noreplymolopt@gmail.com',[data['email']])
        email.send()
        Clear_media()
    ...
    if len(RUNS_QUEUE)<=MAX_PARALLEL_RUNS:
        ...
        if data['xgboost']['isXGBoost']:
            ...
            if not data['xgboost']['isAuto']:
                Model_Training_Script.make_it_train('csv'+str(rId),xg['features'],xg['learningRate'],xg['maxDepth'],xg['lambda'],xg['alpha'],xg['dropRate'],rId)
        ...
        if data['dtr']['isDTR']:
            ...
            if not data['dtr']['isAuto']:
                DecisionTreeRegressor_Manual.make_it_train('csv'+str(rId),tree['features'],tree['maxDepth'],tree['minSample'],tree['minSampleLeaf'],tree['minWeightFraction'],rId)
...
class UserDownloadResultApiView(APIView):
    def get(self,request):
        ...
        with zipfile.ZipFile(f'Result{rId}.zip', 'w', zipfile.ZIP_DEFLATED) as result:
            for f in files:
