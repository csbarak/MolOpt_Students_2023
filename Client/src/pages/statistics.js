...
const Statistics = () => {
  ReactGA.initialize('G-C7R3NTD0Q9');
        ...
        <div className='row'>
          <div className='col-sm'>
            ...
            <ResponsiveContainer width="95%" height={350}>
              <PieChart margin={{top: 0, right: 5, left: 0, bottom: 5,}}>
                <Pie dataKey="value" isAnimationActive={false} data={dataPie} cx={200} cy={150} outerRadius={80} label>
                  {
                    dataPie.map((_, index) => (
          ...
          <div className='col-sm'>
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <FAQCard
                  question='How many visitors visited MolOpt?'
                  answer=  {answers.visitorsCount}
                  isAnswer={isAnswer}
              ...
              <Grid item xs={12}>
                <FAQCard
                  question='How many registered users using MolOpt?'
                  answer={answers.registeredCount}
                  isAnswer={isAnswer}
                  setIsAnswer={setIsAnswer}
                  index={1}
                />
              ...
              <Grid item xs={12}>
                <FAQCard
                  question='How many Admins users MolOpt has?'
                  answer={answers.AdminsCount}
                  isAnswer={isAnswer}
                  setIsAnswer={setIsAnswer}
                  index={2}
                />
              ...
              <Grid item xs={12}>
                <FAQCard
                  question='How many Algorithms runs where executed in total in MolOpt?'
                  answer={answers.AlgosRunsCount}
                  isAnswer={isAnswer}
                  setIsAnswer={setIsAnswer}
                  index={3}
                />
            ...
            {/* Google Analytics */}
            <br/><br/>
            <h4 className="text-decoration-underline fw-lighter">Google Analytics - for admin usage</h4>
            <Button variant="outlined" color="error" onClick={componentDidMount}>Google Analytics Testing</Button>
          </div>
        </div>
      </div>
    </div>
